...
;addition: inscription ---------------------------------------------------------

Local $mUpgradeItem = DllStructCreate('ptr;dword;dword;dword;dword') ; The ptr will hold the address to call
Local $mUpgradeItemPtr = DllStructGetPtr($mUpgradeItem) ; Pointer to struct we will use.
...
Func Initialize($aGW, $bChangeTitle = True, $aUseStringLog = False, $aUseEventSystem = True)
	...
	$mCurrentStatus = MemoryRead(GetScannedAddress('ScanChangeStatusFunction', 33)) ;new addition
	...
	Local $lllTemp = GetScannedAddress('ScanIncreaseAttributeFunction', -96)
	SetValue('IncreaseAttributeFunction', '0x' & Hex($lllTemp, 8))
	SetValue('DecreaseAttributeFunction', '0x' & Hex($lllTemp - 192, 8))
	...
	SetValue('UpgradeItemFunction','0x004574D0') ;not a pattern, merely the address
	...
	DllStructSetData($mSendChat, 2, 0x5D+1)
	...
	;new addition========================================================================
	;;changes add 1 to old value
	DllStructSetData($mmaxattributes, 1, getvalue("CommandPacketSend"))
	DllStructSetData($mmaxattributes, 2, 144)
	DllStructSetData($mmaxattributes, 3, 8);;not changed, was 8
	DllStructSetData($mmaxattributes, 5, 2);;not changed, was 2
	DllStructSetData($mmaxattributes, 22, 2);;not changed, was 2
	DllStructSetData($mmaxattributes, 23, 12);;not changed, was 12
	DllStructSetData($mmaxattributes, 24, 12);;not changed, was 12
	DllStructSetData($msetattributes, 1, getvalue("CommandPacketSend"))
	DllStructSetData($msetattributes, 2, 144)
	DllStructSetData($msetattributes, 3, 9);;changed, was 8
	;new addition========================================================================
...
;  获取全队数据, 各变量每轮须在正文归零; 0xDB不包含箱子等物品0x200
Func GetParty($aAgentArray = 0)
	Local $lReturnArray[1] = [0]
	Local $temp_name, $agentName
	If $aAgentArray==0 Then $aAgentArray = GetAgentArray(0xDB)

	For $i = 1 To $aAgentArray[0]

		; get player/agent name
		$temp_name = GetPlayerName($aAgentArray[$i])
		if $temp_name <> "" Then
			$agentName = $temp_name
		Else
			$agentName = GetAgentName($aAgentArray[$i])
		Endif

		; 下三段所用数列不会归零， 死者不被除名/号， 新人物若同号将不会被标/喊出来

		; 标/喊未曾标/喊过的活猴子, 只在距离$Nightmare_Warn范围内喊
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 3) and ($agentName == 'Dying Nightmare') and (DllStructGetData($aAgentArray[$i], 'HP') > 0) and (GetDistance($aAgentArray[$i]) < $Nightmare_Warn) Then
			if (NOT arrayContains($DNIDArray, DllStructGetData($aAgentArray[$i], 'ID'))) then
				$DNIDArray[0] += 1
				ReDim $DNIDArray[$DNIDArray[0] + 1]
				$DNIDArray[$DNIDArray[0]] = DllStructGetData($aAgentArray[$i], 'ID')
				ChangeTarget($aAgentArray[$i])
				RndSleep(GetPing()+300)
				CallTarget($aAgentArray[$i])
			endif
		EndIf

		;标/喊未曾标/喊过的活牛, 只在距离$Aatxe_Warn范围内标记并发出警告
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 3) and ($agentName == 'Bladed Aatxe') and (DllStructGetData($aAgentArray[$i], 'HP') > 0) and (GetDistance($aAgentArray[$i]) < $Aatxe_Warn) Then
			if (NOT arrayContains($BAIDArray, DllStructGetData($aAgentArray[$i], 'ID'))) then
				$BAIDArray[0] += 1
				ReDim $BAIDArray[$BAIDArray[0] + 1]
				$BAIDArray[$BAIDArray[0]] = DllStructGetData($aAgentArray[$i], 'ID')
				ChangeTarget($aAgentArray[$i])
				RndSleep(GetPing()+300)
				CallTarget($aAgentArray[$i])
				$InterruptPresent += 1
		...
		; 启始部分不要标/喊 grasping darkness
		Local $Dummy_Temp1 = $Grasping_Warn
		if ComputeDistance(1007, 7551, DllStructGetData(GetAgentByID(-2), 'X'),DllStructGetData(GetAgentByID(-2), 'Y')) < 2000 then
			$Dummy_Temp1 = $Grasping_Warn2
		endif

		; 标/喊未曾标/喊过的活grasping darkness, 只在距离2000范围内标记($Grasping_Warn, as entered into $Dummy_Temp1)
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 3) and ($agentName == 'Grasping Darkness') and (DllStructGetData($aAgentArray[$i], 'HP') > 0) and (GetDistance($aAgentArray[$i]) < $Dummy_Temp1) Then
			if (NOT arrayContains($GDIDArray, DllStructGetData($aAgentArray[$i], 'ID'))) then
				$GDIDArray[0] += 1
				ReDim $GDIDArray[$GDIDArray[0] + 1]
				$GDIDArray[$GDIDArray[0]] = DllStructGetData($aAgentArray[$i], 'ID')
				ChangeTarget($aAgentArray[$i])
				RndSleep(GetPing()+300)
				CallTarget($aAgentArray[$i])
			endif
			; 近身时发出警告
			if (GetDistance($aAgentArray[$i]) < $Grasping_Warn2) Then
				$InterruptPresent += 1
			endif
		EndIf

		; 雷达范围内查看是否有mindblade
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 3) and ($agentName == 'Mindblade Spectre')  Then
			$Mindblades = TRUE
			;标/喊未曾标/喊过的活mindblade, 只在距离$mindblade_warn范围内喊
			if (DllStructGetData($aAgentArray[$i], 'HP') > 0) then
				$MindbladeLiveCount += 1
				;  输出最近的mindblade及其距离
				if GetDistance($aAgentArray[$i]) < $NearestMindbladeDistance Then
					$NearestMindbladeDistance = GetDistance($aAgentArray[$i])
					$NearestMindblade = $aAgentArray[$i]
				endif
				;标/喊未曾标/喊过的活mindblade, 只在距离$mindblade_warn范围内标记并发出警告
				if (NOT arrayContains($MSIDArray, DllStructGetData($aAgentArray[$i], 'ID'))) and (GetDistance($aAgentArray[$i]) < $Mindblade_Warn) then
					$MSIDArray[0] += 1
					ReDim $MSIDArray[$MSIDArray[0] + 1]
					$MSIDArray[$MSIDArray[0]] = DllStructGetData($aAgentArray[$i], 'ID')
					ChangeTarget($aAgentArray[$i])
					RndSleep(GetPing()+300)
					CallTarget($aAgentArray[$i])
					$InterruptPresent += 1
		...
		; 记活骷髅总数
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 3) and ($agentName == 'Skeleton of Dhuum') and (DllStructGetData($aAgentArray[$i], 'HP') > 0) Then
			$SkeleCount += 1
		EndIf

		; 记活不速之客总数
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 3) and ($agentName == 'Keeper of Souls') and (DllStructGetData($aAgentArray[$i], 'HP') > 0) Then
			$KeeperCount += 1
		EndIf

		; 纳入需护送的灵， 只加血， 不上加持(需在正文实施)
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 6) and ($agentName == 'New Soul') Then
			$Souls = TRUE
			If (DllStructGetData($aAgentArray[$i], 'HP') < 0.75) Then
				$lReturnArray[0] += 1
				ReDim $lReturnArray[$lReturnArray[0] + 1]
		...
		; 暂不纳入其他灵Tortured Spirit， 因同名可能导致无法离开vale
		...
		; 纳入最多一名守护者, 记守护者总数
		If (DllStructGetData($aAgentArray[$i], 'Allegiance') == 6) and (($agentName == 'Reaper of the Bone Pits') or _
																		($agentName == 'Reaper of the Chaos Planes') or _
																		($agentName == 'Reaper of the Forgotten Vale') or _
																		($agentName == 'Reaper of the Ice Wastes') or _
																		($agentName == 'Reaper of the Labyrinth') or _
																		($agentName == 'Reaper of the Spawning Pools') or _
																		($agentName == 'Reaper of the Twin Serpent Mountains')) Then
				If $ReaperCount = 0 then
					$ReaperCount += 1
					$lReturnArray[0] += 1
					ReDim $lReturnArray[$lReturnArray[0] + 1]
					$lReturnArray[$lReturnArray[0]] = $aAgentArray[$i]
					$ReaperPosition = $lReturnArray[0]
				else
					$ReaperCount += 1
				endif
		EndIf
...
Func CreateCommands()
	...
	_('lea ecx,dword[eax+4]') ; Loads the pointer to eax + 4 bytes (our struct) into ecx, which is the first and only parameter in this case.
	_('call UpgradeItemFunction') ; Call game function.
	_('ljmp CommandReturn') ; Return to gameloop.
...
Func Fn0554();part of GWA2 UNKNOWN, has to do with everyone in party loading
...
Func RndTravel($aMapID)
	;comment/delete for random action
		MoveMap($aMapID, 2, 0, 4)
		return true
	;end comment/delete section
	Local $UseDistricts = 4 ; 7=eu, 8=eu+int, 11=all(incl. asia)
	; Region/Language order: eu-en, eu-fr, eu-ge, eu-it, eu-sp, eu-po, eu-ru, int, asia-ko, asia-ch, asia-ja | [2, 0],[2, 2],[2, 3],[2, 4], [2, 5],[2, 9],[2, 10],
	Local $ComboDistrict[4][2]=[[-2, 0],[1, 0], [3, 0], [4, 0]]
...
Func UpgradeItem($aWeaponBag,$aWeaponSlot,$aModBag,$aModSlot, $aModType) ;Modtype 0 prefix 1 suffix 2 inscription
	...
	DllStructSetData($mUpgradeItem,4,DllStructGetData(GetItemBySlot($aWeaponBag, $aWeaponSlot), 'ID')) ; Weapon Itemid (NOT GetSlotById()!!)
	DllStructSetData($mUpgradeItem,5,DllStructGetData(GetItemBySlot($aModBag, $aModSlot), 'ID')) ; Mod Itemid (NOT GetSlotById()!!)

	Enqueue($mUpgradeItemPtr,0x14) ; Enqueue call, the number after is the size in bytes of the full structure (DllStructGetSize() will work as well)
	...
	SendPacket(0x4,0x7B) ;effect: clicks upgrade button, freezes screen
