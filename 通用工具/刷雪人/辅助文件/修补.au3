	...
	For $i = 1 To DllStructGetData($aBag, 'Slots')
		...
		((GetItemAttribute($lItem) = 29) and (DllStructGetData($lItem, 'Value')<= $gateValue) and _
		  (NOT ((($Requirement = 5) or ($Requirement = 6))  and (GetItemMaxDmg($lItem) > 12) and (GetItemInscr($lItem) <> 0)) ) ) then
				...
				if NOT StartSalvage($lItem) then ;底层需要相应改动
				    out("拆解器用尽")
...
Func CANSELL($aItem)
	...
	if (DllStructGetData($aitem, 'Type')==5) and (GetItemMaxDmg($aitem) <= 20) then ;sell bow with too low max damage
		return True
	EndIf
...
Func CheckRealPlayerIsInOutPost()
	...
	while (NOT $Player_Name)
		...
		Soundplay("铃响.mp3")
		out("因有人而换区")
		;$rezone=1 ;THIS VARIABLE IS DECLARED INDEPENDENT OF THIS FILE, MAY NOT BE SIMPLY COPIED TO OTHER PROJECTS
