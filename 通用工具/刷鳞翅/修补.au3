...
Func CanPickUp($aItem)
	...
	Elseif ($lModelID == 19185) then ;如非鳞翅，但是兽肉， 则:
		Return True ;要求捡起
	Elseif (DllStructGetData($aItem, 'Type') == 35) and (GetScytheMinDmg($aItem) > 6) and (GetScytheMaxDmg($aItem) > 14) and ($lRarity <> $RARITY_WHITE) then ;如武器为镰刀类, 并且最低伤害大于等于7, 同时最高伤害大于等于15, 且非白色, 则:
		Return True ;要求捡起
...
;#ce
	...
	ElseIf ($lRarity == $RARITY_PURPLE) And $PickUpAll Then ;拿紫?
		Return false ;否
