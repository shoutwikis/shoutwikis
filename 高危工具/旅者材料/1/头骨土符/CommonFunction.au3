...
Global $strName = ""
Global $coords[2]
Global $farmItemID = 814
Global $farmItemName = "Skull Juju"
...
GUICtrlCreateLabel($farmItemName, 15, 80, 280)
GUICtrlSetFont(-1, 15)

GUICtrlCreateLabel("Pickups Options:",120,80)
Global $pickupTomes = GUICtrlCreateCheckbox("Elite Tomes",120, 80 + 20)
Global $pickupEvent = GUICtrlCreateCheckbox("Event Items",120, 80 + 40)
Global $pickupMoney = GUICtrlCreateCheckbox("Money",120, 80 + 60)
Global $pickupGold = GUICtrlCreateCheckbox("Goldies",200, 80 + 20)
Global $pickupGreen = GUICtrlCreateCheckbox("Greens",200, 80 + 40)
Global $pickupLock = GUICtrlCreateCheckbox("Lockpicks",200, 80 + 60)

$Start = GUICtrlCreateButton("Start", 30, 110, 75, 25, $WS_GROUP)
GUICtrlSetOnEvent(-1, "gui_eventHandler")

;Rendering
Global Const $Checkbox = GUICtrlCreateCheckbox("Dis. Rend", 30, 138, 75, 17)
...
Func CanPickUp($aitem)
	...
	If $r = $RARITY_Gold And IsChecked($pickupGold) then Return True
	If $r = $RARITY_Green And IsChecked($pickupGold) then Return True

	;Elite Tomes
	If ($ModelID == $ITEM_ID_Assassin_EliteTome Or $ModelID == $ITEM_ID_Dervish_EliteTome Or $ModelID == $ITEM_ID_Elementalist_EliteTome Or $ModelID == $ITEM_ID_Mesmer_EliteTome Or $ModelID == $ITEM_ID_Monk_EliteTome Or $ModelID == $ITEM_ID_Necromancer_EliteTome Or $ModelID == $ITEM_ID_Paragon_EliteTome Or $ModelID == $ITEM_ID_Ranger_EliteTome Or OR $ModelID == $ITEM_ID_Ritualist_EliteTome Or $ModelID == $ITEM_ID_Warrior_EliteTome) And IsChecked($pickupTomes) Then Return True

	;Event items


	If $ModelID == 22751 And IsChecked($pickupLock) Then Return True ; Lockpick
	If $ModelID == 2511 And GetGoldCharacter() < 99000 And IsChecked($pickupMoney) Then Return True	;2511 = Gold Coins

	If $ModelID = $farmItemID Then Return True
	If $ModelID <> $farmItemID Then Return False
