				...
				For $i = 0 To $totalskills
					...
					If $recharge = 0 And $energy >= $intSkillEnergy[$i] And $adrenaline >= ($intSkillAdrenaline[$i]*25 - 25) Then
						...
						If $i = 0 Then
							...
							Until DllStructGetData(GetAgentByID(-1), "LastStrike") = 0x1 OR $variabletosort = 3
						ElseIf $i = 1 Then
							Do
								$energy = GetEnergy(-2)
								If $energy >= $intSkillEnergy[$i] Then
								...
								If DllStructGetData(GetCurrentTarget(), 'Effects') = 0x0010 Then ExitLoop
								If DllStructGetData(GetCurrentTarget(),'Type') = 0x400 then ExitLoop
							Until DllStructGetData(GetAgentByID(-1), "LastStrike") = 0x2 OR $variabletosort = 3
						ElseIf $i = 2 Then
							Do
								$energy = GetEnergy(-2)
								If $energy >= $intSkillEnergy[$i] Then
								...
								If DllStructGetData(GetCurrentTarget(), 'Effects') = 0x0010 Then ExitLoop
								If DllStructGetData(GetCurrentTarget(),'Type') = 0x400 then ExitLoop
							Until DllStructGetData(GetAgentByID(-1), "LastStrike") = 0x3 OR $variabletosort = 3
						Else
							UseSkill($useSkill, $target)
