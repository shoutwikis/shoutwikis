...
Local $mUseSkill = DllStructCreate('ptr;dword;dword')
...
Local $mPacket = DllStructCreate('ptr;dword;dword;dword;dword;dword;dword;dword;dword;dword')
...
Func MemoryOpen($aPID)
	$mHandle[0] = DllOpen('kernel32.dll')
	Local $lOpenProcess = DllCall($mHandle[0], 'int', 'OpenProcess', 'int', 0x1F0FFF, 'int', 1, 'int', $aPID)
	$mHandle[1] = $lOpenProcess[0]
EndFunc   ;==>MemoryOpen

;~ Description: Internal use only.
Func MemoryClose()
	DllCall($mHandle[0], 'int', 'CloseHandle', 'int', $mHandle[1])
	DllClose($mHandle[0])
EndFunc   ;==>MemoryClose
...
Func WriteBinary($aBinaryString, $aAddress)
	...
	Next
	DllCall($mHandle[0], 'int', 'WriteProcessMemory', 'int', $mHandle[1], 'ptr', $aAddress, 'ptr', DllStructGetPtr($lData), 'int', DllStructGetSize($lData), 'int', 0)
EndFunc   ;==>WriteBinary
...
Func MemoryWrite($aAddress, $aData, $aType = 'dword')
	...
	DllCall($mHandle[0], 'int', 'WriteProcessMemory', 'int', $mHandle[1], 'int', $aAddress, 'ptr', DllStructGetPtr($lBuffer), 'int', DllStructGetSize($lBuffer), 'int', '')
EndFunc   ;==>MemoryWrite

;~ Description: Internal use only.
Func MemoryRead($aAddress, $aType = 'dword')
	Local $lBuffer = DllStructCreate($aType)
	DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $aAddress, 'ptr', DllStructGetPtr($lBuffer), 'int', DllStructGetSize($lBuffer), 'int', '')
	Return DllStructGetData($lBuffer, 1)
EndFunc   ;==>MemoryRead

;~ Description: Internal use only.
Func MemoryReadPtr($aAddress, $aOffset, $aType = 'dword')
	Local $lPointerCount = UBound($aOffset) - 1

	Local $lData[2]
	Local $aBuffer = DllStructCreate('dword')

	For $i = 0 To $lPointerCount
		If $i = $lPointerCount Then
			$aBuffer = DllStructCreate($aType)
			$aAddress = $lData[1] + $aOffset[$i]
			DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $aAddress, 'ptr', DllStructGetPtr($aBuffer), 'int', DllStructGetSize($aBuffer), 'int', '')
			$lData[1] = DllStructGetData($aBuffer, 1)
		ElseIf $i = 0 Then
			DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $aAddress, 'ptr', DllStructGetPtr($aBuffer), 'int', DllStructGetSize($aBuffer), 'int', '')
			$lData[1] = DllStructGetData($aBuffer, 1)
		Else
...
Func Initialize($aGW, $bChangeTitle = True, $aUseStringLog = False, $aUseEventSystem = True)
	...
	If $mHandle[1] = 0 Then Return False
	...
	$mBaseOffset = GetScannedAddress('ScanBaseOffset', 1)
	SetValue('PacketLocation', '0x' & Hex(MemoryRead($mBaseOffset - 4), 8))
	...
	SetValue('ZoomStill', '0x' & Hex(GetScannedAddress("ScanZoomStill", -1), 8))
	SetValue('ZoomMoving', '0x' & Hex(GetScannedAddress("ScanZoomMoving", 5), 8))

	$lTemp = GetScannedAddress('ScanEngine', -16)
	SetValue('MainStart', '0x' & Hex($lTemp, 8))
	SetValue('MainReturn', '0x' & Hex($lTemp + 5, 8))
	SetValue('RenderingMod', '0x' & Hex($lTemp + 118, 8))
	SetValue('RenderingModEx', '0x' & Hex($lTemp + 128 + 6, 8))
	...
	SetValue('IncreaseAttributeFunction', '0x' & Hex(GetScannedAddress('ScanIncreaseAttributeFunction', 50), 8))
	...
	DllStructSetData($mSendChat, 2, 0x5D)
...
Func Scan()
	...
	AddPattern('8b47106a00506a016a04e8')
	...
	If $lScanMemory = 0 Then
		$mMemory = DllCall($mHandle[0], 'ptr', 'VirtualAllocEx', 'handle', $mHandle[1], 'ptr', 0, 'ulong_ptr', $mASMSize, 'dword', 0x1000, 'dword', 0x40)
		$mMemory = $mMemory[0]
	...
	If $lScanMemory = 0 Then
		...
		$lThread = DllCall($mHandle[0], 'int', 'CreateRemoteThread', 'int', $mHandle[1], 'ptr', 0, 'int', 0, 'int', GetLabelInfo('ScanProc'), 'ptr', 0, 'int', 0, 'int', 0)
		$lThread = $lThread[0]
		Do
			$lResult = DllCall($mHandle[0], 'int', 'WaitForSingleObject', 'int', $lThread, 'int', 50)
		Until $lResult[0] <> 258
		DllCall($mHandle[0], 'int', 'CloseHandle', 'int', $lThread)
	EndIf
EndFunc   ;==>Scan
...
Func ScanForCharname()
	...
	While $lCurrentSearchAddress < 0x00900000
		Local $lMBI[7]
		DllCall($mHandle[0], 'int', 'VirtualQueryEx', 'int', $mHandle[1], 'int', $lCurrentSearchAddress, 'ptr', DllStructGetPtr($lMBIBuffer), 'int', DllStructGetSize($lMBIBuffer))
		For $i = 0 To 6
		...
		If $lMBI[4] = 4096 Then
			...
			DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lCurrentSearchAddress, 'ptr', DllStructGetPtr($lBuffer), 'int', DllStructGetSize($lBuffer), 'int', '')
			...
			If $lSearch > 0 Then
				...
				DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lTmpAddress + 0x6, 'ptr', DllStructGetPtr($lTmpBuffer), 'int', DllStructGetSize($lTmpBuffer), 'int', '')
...
Func TraderRequest($aModelID, $aExtraID = -1)
	...
	For $lItemID = 1 To $lItemArraySize[1]
		...
		DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lItemPtr[1], 'ptr', DllStructGetPtr($lItemStruct), 'int', DllStructGetSize($lItemStruct), 'int', '')
...
Func MoveTo($aX, $aY, $aRandom = 50)
	...
	Do
		...
		If DllStructGetData($lMe, 'MoveX') == 0 And DllStructGetData($lMe, 'MoveY') == 0 Then
			...
			Move($lDestX, $lDestY, 0)
		EndIf
	Until ComputeDistance(DllStructGetData($lMe, 'X'), DllStructGetData($lMe, 'Y'), $lDestX, $lDestY) < 250 Or $lBlocked > 100
EndFunc   ;==>MoveTo
...
Func Attack($aAgent)
	...
	Return SendPacket(0xC, 0x1F, $lAgentID, 0)
EndFunc   ;==>Attack
...
Func TravelTo($aMapID, $aDis = 0)
	;returns true if successful
	If GetMapID() = $aMapID And $aDis = 0 Then Return True
	ZoneMap($aMapID, $aDis)
...
Func MoveMap($aMapID, $aRegion, $aDistrict, $aLanguage)
	Return SendPacket(0x18, 0xAA, $aMapID, $aRegion, $aDistrict, $aLanguage, 1)
EndFunc   ;==>MoveMap
...
;~ Description: Call and attack target.
Func CallAttack($aTarget)
	Local $lTargetID
	...
	Return SendPacket(0xC, 0x1F, $lTargetID, 1)
EndFunc   ;==>CallAttack
...
Func EnableRendering()
	WriteBinary('7512', GetValue('RenderingMod'))
	WriteBinary('8BE55DC3D8', GetValue('RenderingModEx'))
EndFunc   ;==>EnableRendering

;~ Description: Disable graphics rendering.
Func DisableRendering()
	WriteBinary('9090', GetValue('RenderingMod'))
	WriteDetour('RenderingModEx', 'RenderingModExProc')
...
Func WriteChat($aMessage, $aSender = 'GWA²')
	...
	DllCall($mHandle[0], 'int', 'WriteProcessMemory', 'int', $mHandle[1], 'int', $lAddress, 'ptr', $mWriteChatPtr, 'int', 4, 'int', '')
...
Func SendChat($aMessage, $aChannel = '!')
	...
	DllCall($mHandle[0], 'int', 'WriteProcessMemory', 'int', $mHandle[1], 'int', $lAddress, 'ptr', $mSendChatPtr, 'int', 8, 'int', '')
...
Func UseSkill($aSkillSlot, $aTarget)
	Local $lTargetID
	...
	For $i = 0 To $lCount[1] - 1
		...
		If $lBuffer[1] == GetHeroID($aHeroNumber) Then
			...
			For $j = 0 To $lBuffCount - 1
				...
				DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lBuffStructAddress[0], 'ptr', DllStructGetPtr($lBuffStruct), 'int', DllStructGetSize($lBuffStruct), 'int', '')
...
Func SetSkillbarSkill($aSlot, $aSkillID)
	Return SendPacket(0x14, 0x54, GetMyID(), $aSlot - 1, $aSkillID, 0)
...
Func ChangeSecondProfession($aProfession)
	Return SendPacket(0xC, 0x3A, GetMyID(), $aProfession)
...
Func ChangeMaxZoom($aZoom = 750)
	MemoryWrite(GetValue('ZoomStill'), $aZoom, "float")
	MemoryWrite(GetValue('ZoomMoving'), $aZoom, "float")
EndFunc   ;==>ChangeMaxZoom
#EndRegion Misc

;~ Description: Internal use only.
Func Enqueue($aPtr, $aSize)
	DllCall($mHandle[0], 'int', 'WriteProcessMemory', 'int', $mHandle[1], 'int', 256 * $mQueueCounter + $mQueueBase, 'ptr', $aPtr, 'int', $aSize, 'int', '')
	If $mQueueCounter = $mQueueSize Then
		$mQueueCounter = 0
	Else
		$mQueueCounter = $mQueueCounter + 1
...
Func GetBag($aBag)
	...
	DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lBagPtr[1], 'ptr', DllStructGetPtr($lBagStruct), 'int', DllStructGetSize($lBagStruct), 'int', '')
	Return $lBagStruct
EndFunc   ;==>GetBag
...
Func GetItemBySlot($aBag, $aSlot)
	...
	Local $lItemStruct = DllStructCreate('long id;long agentId;byte unknown1[4];ptr bag;ptr modstruct;long modstructsize;ptr customized;byte unknown2[4];byte type;byte unknown3;short extraId;short value;byte unknown4[2];short interaction;long modelId;ptr modString;byte unknown5[4];ptr NameString;byte unknown6[15];byte quantity;byte equipped;byte unknown7[1];byte slot')
	DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', DllStructGetData($lBag, 'ItemArray') + 4 * ($aSlot - 1), 'ptr', DllStructGetPtr($lBuffer), 'int', DllStructGetSize($lBuffer), 'int', '')
	DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', DllStructGetData($lBuffer, 1), 'ptr', DllStructGetPtr($lItemStruct), 'int', DllStructGetSize($lItemStruct), 'int', '')
	Return $lItemStruct
...
Func GetAgentByID($aAgentID = -2)
	...
	;Offsets: 0x2C=AgentID 0xF4=PlayerNumber 0114=Energy Pips
	...
	DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lAgentPtr, 'ptr', DllStructGetPtr($lAgentStruct), 'int', DllStructGetSize($lAgentStruct), 'int', '')
	Return $lAgentStruct
...
Func GetNearestEnemyToAgent($aAgent = -2)
	...
	For $i = 1 To GetMaxAgents()
		...
		$lDistance = sqrt((DllStructGetData($aAgent, 'X') - DllStructGetData($lAgentToCompare, 'X')) ^ 2 + (DllStructGetData($aAgent, 'Y') - DllStructGetData($lAgentToCompare, 'Y')) ^ 2)
		If $lDistance < $lNearestDistance Then
			$lNearestAgent = $lAgentToCompare
			$lNearestDistance = $lDistance
		EndIf
...
Func GetNearestAgentToCoords($aX, $aY)
	...
	For $i = 1 To GetMaxAgents()
		$lAgentToCompare = GetAgentByID($i)
		If DllStructGetData($lAgentToCompare, 'HP') < 0.005 Then ContinueLoop
...
Func GetNearestSignpostToAgent($aAgent = -2)
	...
	For $i = 1 To GetMaxAgents()
		$lAgentToCompare = GetAgentByID($i)
		If DllStructGetData($lAgentToCompare, 'Type') <> 0x200 Then ContinueLoop
...
Func GetNearestNPCToAgent($aAgent)
	...
	For $i = 1 To GetMaxAgents()
		$lAgentToCompare = GetAgentByID($i)
		If DllStructGetData($lAgentToCompare, 'Type') <> 0xDB Then ContinueLoop
		If DllStructGetData($lAgentToCompare, 'Allegiance') <> 6 Then ContinueLoop
		If DllStructGetData($lAgentToCompare, 'HP') <= 0 Then ContinueLoop
		If BitAND(DllStructGetData($lAgentToCompare, 'Effects'), 0x0010) > 0 Then ContinueLoop
...
;~ 		If $aCanPickUp And Not GetCanPickUp($lAgentToCompare) Then ContinueLoop
;~ 		$lDistance = (DllStructGetData($aAgent, 'X') - DllStructGetData($lAgentToCompare, 'X')) ^ 2 + (DllStructGetData($aAgent, 'Y') - DllStructGetData($lAgentToCompare, 'Y')) ^ 2
...
Func GetParty()
	...
	For $i = 1 To GetMaxAgents()
		...
		If DllStructGetData($lAgent, 'Allegiance') == 1 Then
			If BitAND(DllStructGetData($lAgent, 'TypeMap'), 131072) Then
				$lReturnArray[0] += 1
				ReDim $lReturnArray[$lReturnArray[0] + 1]
				$lReturnArray[$lReturnArray[0]] = $lAgent
			EndIf
		EndIf
	Next
	Return $lReturnArray
...
Func GetAgentName($aAgent)
	...
	Local $lAddress = $mStringLogBase + 128 * $lAgentID
...
Func GetSkillbar($aHeroNumber = 0)
	...
	For $i = 0 To GetHeroCount()
		...
		DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lSkillbarStructAddress[0], 'ptr', DllStructGetPtr($lSkillbarStruct), 'int', DllStructGetSize($lSkillbarStruct), 'int', '')
...
Func GetSkillByID($aSkillID)
	...
	DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lSkillStructAddress, 'ptr', DllStructGetPtr($lSkillStruct), 'int', DllStructGetSize($lSkillStruct), 'int', '')
	Return $lSkillStruct
		...
		If $lBuffer[1] == GetHeroID($aHeroNumber) Then
			...
			If $aSkillID = 0 Then
				...
				For $i = 0 To $lEffectCount[1] - 1
					...
					DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lEffectStructAddress[1], 'ptr', DllStructGetPtr($lReturnArray[$i + 1]), 'int', 24, 'int', '')
				Next
			...
			Else
				...
				For $i = 0 To $lEffectCount[1] - 1
					DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lEffectStructAddress[0] + 24 * $i, 'ptr', DllStructGetPtr($lReturn), 'int', 24, 'int', '')
...
Func GetEffectTimeRemaining($aEffect)
	If Not IsDllStruct($aEffect) Then Return 0
...
Func GetQuestByID($aQuestID = 0)
	...
	For $i = 0 To $lQuestLogSize[1]
		...
		DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $lQuestPtr[0], 'ptr', DllStructGetPtr($lQuestStruct), 'int', DllStructGetSize($lQuestStruct), 'int', '')
...
Func TolSleep($aAmount, $aTolerance = 50)
...
Func Event($hwnd, $msg, $wparam, $lparam)
	Switch $lparam
		Case 0x1
			DllCall($mHandle[0], 'int', 'ReadProcessMemory', 'int', $mHandle[1], 'int', $wparam, 'ptr', $mSkillLogStructPtr, 'int', 16, 'int', '')
		...
		Case 0x4
			Local $lMessage = MemoryRead($wparam, 'wchar[500]')
			...
			Switch StringLeft($lMessage, 5)
				...
				Case '<quot'
				Case Else
					Call($mChatReceive, 'Global', '', $lMessage)
			EndSwitch
		Case 0x5
			Call($mLoadFinished)
	EndSwitch
...
Func CreateData()
	...
	_('StringLogBase/' & 128 * GetValue('StringLogSize'))
...
;~ Description: Internal use only.
Func CreateRenderingModEx()
	_('RenderingModExProc:')
	_('mov esp,ebp')
	_('pop ebp')
	_('push 1')
	_('call dword[Sleep]')
	_('retn')
EndFunc   ;==>CreateRenderingModEx
...
Func _($aASM)
	...
	Select
		...
		Case StringRegExp($aASM, 'cmp ecx,[a-z,A-Z]{4,}')
			$mASMSize += 6
			$mASMString &= '81F9[' & StringRight($aASM, StringLen($aASM) - 8) & ']'
		Case StringRegExp($aASM, 'cmp ebx,[a-z,A-Z]{4,}')
			$mASMSize += 6
			$mASMString &= '81FB[' & StringRight($aASM, StringLen($aASM) - 8) & ']'
		Case StringRegExp($aASM, 'cmp eax,[a-z,A-Z]{4,}')
			$mASMSize += 5
			$mASMString &= '3D[' & StringRight($aASM, StringLen($aASM) - 8) & ']'
		Case StringRegExp($aASM, 'add eax,[a-z,A-Z]{4,}')
			$mASMSize += 5
		...
		Case StringRegExp($aASM, 'mov dword[[][a-z,A-Z]{4,}[]],0')
			$mASMSize += 10
			$mASMString &= 'C705[' & StringMid($aASM, 11, StringLen($aASM) - 13) & ']00000000'
		Case StringRegExp($aASM, 'mov dword[[][a-z,A-Z]{4,}[]],1')
			$mASMSize += 10
			$mASMString &= 'C705[' & StringMid($aASM, 11, StringLen($aASM) - 13) & ']01000000'
		Case StringRegExp($aASM, 'mov dword[[][a-z,A-Z]{4,}[]],2')
			$mASMSize += 10
			$mASMString &= 'C705[' & StringMid($aASM, 11, StringLen($aASM) - 13) & ']02000000'
...
Func GetLabelInfo($aLabel)
	...
	If $lValue = -1 Then MsgBox(0, 'Label', 'Label: ' & $aLabel & ' not provided')
