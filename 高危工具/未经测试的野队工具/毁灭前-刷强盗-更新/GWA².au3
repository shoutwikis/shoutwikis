...
Func Initialize($aGW, $bChangeTitle = True, $aUseStringLog = true, $aUseEventSystem = True)
	...
	DllStructSetData($mSendChat, 2, 0x6A)
	...
	DllStructSetData($mSetAttributes, 3, 0x11)
...
Func IdentifyItem($aItem)
	...
	SendPacket(0xC, 0x72, $lIDKit, $lItemID)
...
Func EquipItem($aItem)
	...
	Return SendPacket(0x8, 0x37, $lItemID)
EndFunc   ;==>EquipItem
...
Func UseItem($aItem)
	...
	Return SendPacket(0x8, 0x84, $lItemID)
EndFunc   ;==>UseItem
...
Func PickUpItem($aItem)
	...
	Return SendPacket(0xC, 0x46, $lAgentID, 0)
EndFunc   ;==>PickUpItem
...
Func DropItem($aItem, $aAmount = 0)
	...
	Return SendPacket(0xC, 0x33, $lItemID, $lAmount)
EndFunc   ;==>DropItem
...
Func MoveItem($aItem, $aBag, $aSlot)
	...
	Return SendPacket(0x10, 0x78, $lItemID, $lBagID, $aSlot - 1)
EndFunc   ;==>MoveItem

;~ Description: Accepts unclaimed items after a mission.
Func AcceptAllItems()
	Return SendPacket(0x8, 0x79, DllStructGetData(GetBag(7), 'ID'))
...
Func DropGold($aAmount = 0)
	...
	Return SendPacket(0x8, 0x36, $lAmount)
EndFunc   ;==>DropGold
...
Func ChangeGold($aCharacter, $aStorage)
	Return SendPacket(0xC, 0x82, $aCharacter, $aStorage)
EndFunc   ;==>ChangeGold
#EndRegion Item
...
Func LockHeroTarget($aHeroNumber, $aAgentID = 0) ;$aAgentID=0 Cancels Lock
	...
	Return SendPacket(0xC, 0x19, $lHeroID, $aAgentID)
...
Func SetHeroAggression($aHeroNumber, $aAggression) ;0=Fight, 1=Guard, 2=Avoid
	Local $lHeroID = GetHeroID($aHeroNumber)
	Return SendPacket(0xC, 0x18, $lHeroID, $aAggression)
...
Func ChangeHeroSkillSlotState($aHeroNumber, $aSkillSlot)
	Return SendPacket(0xC, 0x1D, GetHeroID($aHeroNumber), $aSkillSlot - 1)
...
Func SetDisplayedTitle($aTitle = 0)
	If $aTitle Then
		Return SendPacket(0x8, 0x5E, $aTitle)
	Else
		Return SendPacket(0x4, 0x5F)
	EndIf
...
Func GoPlayer($aAgent)
	...
	Return SendPacket(0x8, 0x3A, $lAgentID)
EndFunc   ;==>GoPlayer
...
Func GoNPC($aAgent)
	...
	Return SendPacket(0xC, 0x40, $lAgentID)
EndFunc   ;==>GoNPC
...
Func GoSignpost($aAgent)
	...
	Return SendPacket(0xC, 0x58, $lAgentID, 0)
EndFunc   ;==>GoSignpost
...
Func Attack($aAgent, $aCallTarget = False)
	...
	Return SendPacket(0xC, 0x2D, $lAgentID, $aCallTarget)
EndFunc   ;==>Attack
...
Func TravelGH()
    ...
    SendPacket(0x18, 0xB7, MemoryRead($lGH[1] + 0x64), MemoryRead($lGH[1] + 0x68), MemoryRead($lGH[1] + 0x6C), MemoryRead($lGH[1] + 0x70), 1)
    Return WaitMapLoading()
EndFunc   ;==>TravelGH

;~ Description: Leave your guild hall.
Func LeaveGH()
	SendPacket(0x8, 0xB9, 1)
	Return WaitMapLoading()
EndFunc   ;==>LeaveGH
#EndRegion Travel
...
Func AcceptQuest($aQuestID)
	Return SendPacket(0x8, 0x42, '0x008' & Hex($aQuestID, 3) & '01')
EndFunc   ;==>AcceptQuest

;~ Description: Accept the reward for a quest.
Func QuestReward($aQuestID)
	Return SendPacket(0x8, 0x42, '0x008' & Hex($aQuestID, 3) & '07')
EndFunc   ;==>QuestReward

;~ Description: Abandon a quest.
Func AbandonQuest($aQuestID)
	Return SendPacket(0x8, 0x13, $aQuestID)
...
Func CallTarget($aTarget)
	...
	Return SendPacket(0xC, 0x29, 0xA, $lTargetID)
EndFunc   ;==>CallTarget
		...
		If $lBuffer[1] == GetHeroID($aHeroNumber) Then
			...
			For $j = 0 To $lBuffCount - 1
				...
				If (DllStructGetData($lBuffStruct, 'SkillID') == $aSkillID) And (DllStructGetData($lBuffStruct, 'TargetId') == ConvertID($aAgentID)) Then
					Return SendPacket(0x8, 0x30, DllStructGetData($lBuffStruct, 'BuffId'))
					ExitLoop 2
				EndIf
			Next
		EndIf
...
Func DonateFaction($aFaction)
	If StringLeft($aFaction, 1) = 'k' Then
		Return SendPacket(0x10, 0x3C, 0, 0, 0x1388)
	ElseIf StringLeft($aFaction, 1) = 'l' Then
		Return SendPacket(0x10, 0x3C, 0, 1, 0x1388)
	EndIf
EndFunc   ;==>DonateFaction

;~ Description: Open a dialog.
Func Dialog($aDialogID)
	Return SendPacket(0x8, 0x42, $aDialogID)
EndFunc   ;==>Dialog
...
Func SetSkillbarSkill($aSlot, $aSkillID, $aHeroNumber = 0)
	Return SendPacket(0x14, 0x62, GetHeroID($aHeroNumber), $aSlot - 1, $aSkillID, 0)
...
Func LoadSkillBar($aSkill1 = 0, $aSkill2 = 0, $aSkill3 = 0, $aSkill4 = 0, $aSkill5 = 0, $aSkill6 = 0, $aSkill7 = 0, $aSkill8 = 0, $aHeroNumber = 0)
	SendPacket(0x2C, 0x63, GetHeroID($aHeroNumber), 8, $aSkill1, $aSkill2, $aSkill3, $aSkill4, $aSkill5, $aSkill6, $aSkill7, $aSkill8)
...
Func ChangeSecondProfession($aProfession, $aHeroNumber = 0)
	Return SendPacket(0xC, 0x48, GetHeroID($aHeroNumber), $aProfession)
